---
title: "GoM clustering of genome locations using chromatin marks"
author: "Kaixuan Luo"
date: 2017-02-28
...
cluster_GoM_joint <- function(data.m, K = 5, sample_names = '', col_palette = RColorBrewer::brewer.pal(8, "Accent"), cluster_heatmap = F, order_sample = F, fill_empty = T, flip_strand = F){
  ...
  for(i in 1:num_samples){
    ...
    range(idx_sample)
    omega_mat_plot <- omega_mat[idx_sample, ]
    
    if(flip_strand){
      omega_mat_plot <- apply(omega_mat_plot, 2, rev)
    }
...
## load data
```{r load_data, echo=FALSE, cache=F}
...
cluster_GoM_joint(data.m, K = 2, sample_names = cell_type_list, col_palette, cluster_heatmap = F, order_sample = F, fill_empty = T, flip_strand = T)
```

### 3 clusters
```{r GoM_3, echo=FALSE}
cluster_GoM_joint(data.m, K = 3, sample_names = cell_type_list, col_palette, cluster_heatmap = F, order_sample = F, fill_empty = T, flip_strand = T)
```

### 4 clusters
```{r GoM_4, echo=FALSE}
cluster_GoM_joint(data.m, K = 4, sample_names = cell_type_list, col_palette, cluster_heatmap = F, order_sample = F, fill_empty = T, flip_strand = T)
```

### 5 clusters
```{r GoM_5, echo=FALSE}
cluster_GoM_joint(data.m, K = 5, sample_names = cell_type_list, col_palette, cluster_heatmap = F, order_sample = F, fill_empty = T, flip_strand = T)
```

### 6 clusters
```{r GoM_6, echo=FALSE}
cluster_GoM_joint(data.m, K = 6, sample_names = cell_type_list, col_palette, cluster_heatmap = F, order_sample = F, fill_empty = T, flip_strand = T)
```

### 7 clusters
```{r GoM_7, echo=FALSE}
cluster_GoM_joint(data.m, K = 7, sample_names = cell_type_list, col_palette, cluster_heatmap = F, order_sample = F, fill_empty = T, flip_strand = T)
```
